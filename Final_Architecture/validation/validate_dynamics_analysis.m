...
report.gravity = analyzeGravityLoading(robot,struct( ...
    ...
    'numStarts',1,'maxFunctionEvaluations',200));
catalog = robot.gravityPoses;
report.gravityPoses = analyzeGravityLoading(robot,struct( ...
    'gravity',[0 0 -9.81],'q',q0,'includeCandidatePoses',true, ...
    'mode',"poses"));
...
report.gravityCatalogIncluded = report.gravity.candidatePoseCount == ...
    size(catalog.q,1) && all(ismember(catalog.names,report.gravityPoses.poseNames));
...
report.gravityCatalogRepeatable = isequal(report.gravity.q, ...
    ...
    'q',q0,'numSamples',20,'seed',19, ...
    'numStarts',1,'maxFunctionEvaluations',200)).q);
report.gravityCatalogExclusion = analyzeGravityLoading(robot,struct( ...
    'gravity',[0 0 -9.81],'q',q0, ...
    'includeCandidatePoses',false,'mode',"poses")).candidatePoseCount == 0;
report.gravityPeakShape = isequal(size(report.gravity.q), ...
    [robot.structure.dof robot.structure.dof]) && ...
    isequal(size(report.gravity.torque),size(report.gravity.q)) && ...
    numel(report.gravity.poseNames) == robot.structure.dof && ...
    ~report.gravity.meta.globalMaximumCertified;
limits = robot.params.joints.positionLimits;
optimizedQ = report.gravity.qAtMaxAbs;
...
for j = 1:robot.structure.dof
    actual = gravityTorque(optimizedModel,report.gravity.qAtMaxAbs(j,:));
    report.maxOptimizedPoseTorqueError_Nm = max( ...
        report.maxOptimizedPoseTorqueError_Nm, ...
        abs(actual(j)-report.gravity.signedTorqueAtMaxAbs_Nm(j)));
end
report.gravitySummaryMatches = max(abs( ...
    abs(diag(report.gravity.torque)) - ...
    report.gravity.summaryTable.MaxFoundAbsGravityTorque_Nm)) < 1e-8;
t = [0; 0.3; 1.1; 2.0];
q = repmat(q0,4,1);
...
expectedStatic = repmat(report.gravityPoses.torque(1,:),4,1);
...
report.pass = report.numerical.pass && report.oneLink.pass && ...
    ...
    report.gravityCatalogExclusion && report.gravityPeakShape && ...
    report.gravitySummaryMatches && ...
...
fprintf('Per-joint gravity peak summary matches returned poses: %d\n', ...
    report.gravitySummaryMatches);
